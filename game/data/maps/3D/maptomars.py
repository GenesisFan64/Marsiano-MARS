...
TAG_THISFOLDER    = "game/data/maps/3D/"       # Base folder
...
#list_faces    = list()
list_mtrl     = list()
map_file      = open(object_name+"/"+TAG_OBJECTSDIR+"/layout.txt","r")
out_laydata   = open(object_name+"/out_map.bin","wb")
out_layout    = open(object_name+"/md_layout.asm","w")    # GENESIS SIDE, LABELS POINT TO SH2 VIEW
out_layoutm   = open(object_name+"/mars_data.asm","w") # SH2 SIDE
...
for curr_rdpz in range(0,num_pz):
  ...
  model_file    = open(object_name+"/"+TAG_OBJECTSDIR+"/"+mappz_list[curr_pz]+".obj","r")
  material_file = open(object_name+"/"+TAG_OBJECTSDIR+"/"+mappz_list[curr_pz]+".mtl","r")	# CHECK BELOW
  out_head      = open(object_name+"/"+"pz/"+mappz_list[curr_pz]+".asm","w")	# header
  out_vertices  = open(object_name+"/"+"pz/"+mappz_list[curr_pz]+"_vert.bin","wb")	# vertices (points)
  out_faces     = open(object_name+"/"+"pz/"+mappz_list[curr_pz]+"_face.bin","wb")	# faces
  #out_vertex    = open(object_name+"_vrtx.bin","wb")	# texture vertex (MOVED)
  out_mtrl      = open(object_name+"/"+"pz/"+mappz_list[curr_pz]+"_mtrl.asm","w")
          ...
          if mtltext.find("newmtl "+mtlname) == False:
              ...
              while i:
                ...
                if b.find("map_Kd") == False:
                    ...
                    tex_file = open(object_name+"/"+TAG_OBJECTSDIR+"/"+tex_fname,"rb")
                    ...
                    if image_type == 1:
                      ...
                      if int(CONVERT_TEX) == True:
                        ...
                        has_img = True
                        output_file = open(object_name+"/mtrl/"+outname+"_pal.bin","wb")
                        d = pal_len
                        while d:
                          d -= 1
                        ...
                        art_file = open(object_name+"/mtrl/"+outname+"_art.bin","wb")
                        b = img_height
                        e = 0
                        while b:
                          c = img_width
  ...
  if cntr != 0:
    out_vertex = open(object_name+"/pz/"+mappz_list[curr_pz]+"_vrtx.bin","wb")	# texture vertex
